...
void TrajectoryGeneration::fileInput(const string str, const float id)
{
	...
	ROS_INFO("Look Up Table setting....id:%.1f",id);
	if( !ifs ){
		cout << "Error:Non file!!!!!¥n";
	}else{
		float a;
		list<float> array;
		GoalPoint p;
								...
								float dt, float tolerance, float max_count)
{
	...
	while(c.norm() > tolerance && count<max_count){
		...
		for(float time=0; time<cntl_out.velocity.tf+dt; time+=dt){
		...
		Eigen::Matrix3f J;
		//makeJacob(J, dt, cntl_out, 0.001, 0.001, 0.1);
		makeJacob(J, 0.2, cntl_out, 0.005, 0.005, 0.1);
...
TrajectoryGeneration::estimateTime(const VelParam & upv, const CurvParam& upk){
	...
	float t0=fabs( (upv.vt-upv.v0)/upv.a0);
	float td=fabs( (upv.vf-upv.vt)/upv.af);
	
	float s0=fabs(upv.vt+upv.v0)*t0/2;
	float sd=fabs(upv.vt+upv.vf)*td/2;

	float st=upk.sf-s0-sd;
	float tt=st/fabs(upv.vt);
	float t=t0+td+tt;
...
TrajectoryGeneration::motionModel(Eigen::VectorXf& x_next, const Eigen::VectorXf& x, const CntlParam& cntl, float dt, float time)
{
	...
	out(0)=x(0)+x(3)*cos(x(2))*dt;
	out(1)=x(1)+x(3)*sin(x(2))*dt;
...
TrajectoryGeneration::terminalState(Eigen::Vector3f& x, float k1, float kf, float sf, const CntlParam& cntl, float dt)
{
	...
	for(float time=0; time<ct.velocity.tf+dt; time+=dt){
